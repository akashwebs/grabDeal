...
This will:
- Start Express server on `ttps://grabdeal-server.vercel.app`
- Connect to MongoDB
...
All API endpoints are available at `ttps://grabdeal-server.vercel.app/api`
...
### Frontend (.env.local)
```
NEXT_PUBLIC_API_URL=ttps://grabdeal-server.vercel.app/api
```
...
```bash
curl -X POST ttps://grabdeal-server.vercel.app/api/offers \
